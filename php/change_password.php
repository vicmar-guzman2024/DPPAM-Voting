...
<body>
  ...
  <section class="signInContainer d-flex flex-column justify-content-center align-items-center" style="height: 100vh;">  
    ...
    <div class="containerOfLeftRight row row-cols-lg-2 row-cols-1 justify-content-between align-items-start">
        ...
        <form class="col-lg-5 col-12 p-4 rightContainer me-5" action="reset_password_process.php" method="POST">
    <h4 class="mb-4">Change Password</h4>
    <input type="hidden" name="password_token"  value="<?php if(isset($_GET['token'])){echo $_GET['token'];} ?>" class="form-control" placeholder="Email Address" required>
    <div class="input-group mb-4">
        <span class="input-group-text" id="email"><i class="fa fa-user"></i></span>
        <input type="text" name="email" id="email" value="<?php if(isset($_GET['email'])){echo $_GET['email'];} ?>" class="form-control" placeholder="Email Address" required>
    </div>
    <div class="input-group mb-4">
        <span class="input-group-text" id="new_password"><i class="fa fa-lock"></i></span>
        <input type="password" name="new_password" id="new_password" class="form-control" placeholder="Enter New Password" required>
    </div>
    <div class="input-group mb-4">
        <span class="input-group-text" id="password"><i class="fa fa-lock"></i></span>
        <input type="password" name="confirm_password" id="confirm_password" class="form-control" placeholder="Re-enter Confirm Password " required>
    </div>


    <div>
        <button class="btn btn-primary w-100" name="change_pass_btn" type="submit">Change Password</button>
    </div>
</form>
